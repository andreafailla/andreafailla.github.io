---
title: Network Analysis and Visualization
summary: Remote training within the YADESA project
date: 2026-03-27
type: docs
math: false
tags:
  - Network Science
image:
  caption: 
---

This is the page for the Network Analysis and Visualization class within the YADESA project. The class is held remotely on Zoom.

### Class Description
This class will last 4 hours and will introduce the audience to the following topics:
- Overview of Network Analysis: networks, measures and tasks
- Network Analysis in Python with NetworkX
- Network Visualization with [Gephi](https://gephi.org/)


Slides: [here](https://docs.google.com/presentation/d/1gPkiAmuA6-4UKGjPZnWeOUJoVl9bzle-_N9oibcOKRw/edit?usp=sharing) (adapted from Prof. [Giulio Rossetti](https://giuliorossetti.github.io/)'s material)

Notebook: [here](../../../static/uploads/notebooks/yadesa-SNA.ipynb)

Data: ...

Book with network theory/practice explained in simple terms and easy tone: [Atlas for the Aspiring Network Scientist](https://www.networkatlas.eu/)

More narrative books:
- A.-L. Barabasi. "Linked". PLUME, Penguin Group, 2002.
- Duncan J. Watts.  "Six Degrees: The Science of a Connected Age." Norton, New York,  2003.



#### Feedback
I am looking for ways to improve my teaching skills, and I welcome any feedback from the students. 
I'd really appreciate if you could answer two short questions anonymously [here](https://docs.google.com/forms/d/e/1FAIpQLSf9Mv-PsLVlaPOXdNni51Mb4oSlb03faGYrzHfRuQYuHFCijQ/viewform?usp=publish-editor) (should take less than 5 minutes).